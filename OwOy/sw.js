...
async function handleRequest(event) {
 ...
 const isScramAsset = reqUrl.pathname.includes('/scram/') || reqUrl.pathname.endsWith('.wasm') || reqUrl.pathname.endsWith('.wasm.wasm');
 const isBareAsset = reqUrl.pathname.includes('/baremux/') || reqUrl.pathname.includes('/libcurl/') || reqUrl.pathname.includes('/favicon') || reqUrl.pathname.endsWith('.ico') || reqUrl.pathname.endsWith('.png');
 // Never proxy/route local static assets; it can corrupt binary wasm or return HTML on 404.
 if (isSameOrigin && (isScramAsset || isBareAsset)) {
   // Ensure wasm has correct content-type for instantiateStreaming.
   if (isScramAsset && (reqUrl.pathname.endsWith('.wasm') || reqUrl.pathname.endsWith('.wasm.wasm'))) {
     const resp = await fetch(event.request);
     const ct = (resp.headers.get('content-type') || '');
     // If server already serves application/wasm, keep it.
     if (ct.includes('application/wasm')) return resp;
     // Don't try to "fix" non-OK responses (404 HTML etc.).
     if (!resp.ok) return resp;
     // Verify magic header (\0asm) before rewriting content-type.
     const buf = await resp.arrayBuffer();
     const u8 = new Uint8Array(buf);
     const isWasmMagic = u8.length >= 4 && u8[0] === 0x00 && u8[1] === 0x61 && u8[2] === 0x73 && u8[3] === 0x6d;
     if (!isWasmMagic) return resp;
